...
const cssTemp = `
  <% for (let item of fontList) {%>
    @font-face {
      ...
      src: url('http://47.96.150.44:8080/assets/font/<%= item.alias+'.'+ext %>') format('<%= item.fontMeta[ext].format %>');
      <% } %>
    }
  <% } %>
`;
...
const getFontList = async () => {
  ...
  const results = fs.readFileSync(resolve("./t.txt"), {encoding: 'utf-8'}).split(/[\n\t]/);
  results.forEach((path) => {

    const pathLists = path.trim().split(".")
...
const createFile = (data) => {
  ...
  fs.writeFileSync(resolve('./font1.scss'),cssStr)
  fs.writeFileSync(resolve('./fontMap1.js'),jsStr)
