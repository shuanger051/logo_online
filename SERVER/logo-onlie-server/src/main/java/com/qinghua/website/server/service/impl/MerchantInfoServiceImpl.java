...
public class MerchantInfoServiceImpl implements MerchantInfoService {
        ...
        @Override
        public Integer saveMerchantInfo(MerchantInfoDTO bean) {
            //根据身份证号校验是否重复
            ...
            return merchantInfoMapper.saveMerchantInfo(bean);
        }

        @Override
        public Integer deleteMerchantInfoById(Long id) {
            Preconditions.checkNotNull(id, "参数:ID不能为空");
            //判断当前商户下是否有商铺信息
        ...
        @Transactional(propagation = Propagation.REQUIRED)
        @Override
        public Integer updateMerchantInfoById(MerchantInfoDTO bean) {
            Preconditions.checkNotNull(bean.getId(), "参数:ID不能为空");
            return merchantInfoMapper.updateMerchantInfoById(bean);
        }
}
